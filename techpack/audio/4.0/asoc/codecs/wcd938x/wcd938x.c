...
static int wcd938x_init_reg(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, WCD938X_SLEEP_CTL, 0x80, 0x80);
	/* 1 msec delay as per HW requirement */
	usleep_range(1000, 1010);
	snd_soc_update_bits(codec, WCD938X_SLEEP_CTL, 0x40, 0x40);
	/* 1 msec delay as per HW requirement */
	usleep_range(1000, 1010);
	snd_soc_update_bits(codec, WCD938X_LDORXTX_CONFIG,
								0x10, 0x00);
	snd_soc_update_bits(codec, WCD938X_BIAS_VBG_FINE_ADJ,
								0xF0, 0x80);
	snd_soc_update_bits(codec, WCD938X_ANA_BIAS, 0x80, 0x80);
	...
	snd_soc_update_bits(codec,
				      ...
				      0x1F, 0x15);
	snd_soc_update_bits(codec, WCD938X_HPH_REFBUFF_UHQA_CTL,
				      0xC0, 0x80);
	snd_soc_update_bits(codec, WCD938X_DIGITAL_CDC_DMIC_CTL,
				      0x02, 0x02);
	snd_soc_update_bits(codec,
...
static int wcd938x_set_port_params(struct snd_soc_codec *codec,
			...
			u8 *port_type, u8 path)
{
	...
	case CODEC_TX:
		...
		break;
	default:
		dev_err(codec->dev, "%s Invalid path selected %u\n",
					__func__, path);
		return -EINVAL;
	}
...
static int wcd938x_rx_connect_port(struct snd_soc_codec *codec,
...
static int wcd938x_rx_clk_enable(struct snd_soc_codec *codec)
{
...
static int wcd938x_rx_clk_disable(struct snd_soc_codec *codec)
{
	struct wcd938x_priv *wcd938x = snd_soc_codec_get_drvdata(codec);
...
/*
 * wcd938x_soc_get_mbhc: get wcd938x_mbhc handle of corresponding codec
 * @codec: handle to snd_soc_codec *
 *
 * return wcd938x_mbhc handle or error code in case of failure
 */
struct wcd938x_mbhc *wcd938x_soc_get_mbhc(struct snd_soc_codec *codec)
{
				       ...
				       int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		wcd938x->ear_rx_path =
			snd_soc_read(
				codec, WCD938X_DIGITAL_CDC_EAR_PATH_CTL);
		...
		wcd_cls_h_fsm(codec, &wcd938x->clsh_info,
			       ...
			       int event)
{
	...
	if (mode == CLS_H_LOHIFI || mode == CLS_H_ULP ||
		...
		wcd938x_rx_connect_port(codec, CLSH,
			      ...
			      int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		wcd938x_rx_connect_port(codec, HPH_L, true);
		if (wcd938x->comp1_enable)
			wcd938x_rx_connect_port(codec, COMP_L, true);
		break;
	case SND_SOC_DAPM_POST_PMD:
		wcd938x_rx_connect_port(codec, HPH_L, false);
		if (wcd938x->comp1_enable)
			wcd938x_rx_connect_port(codec, COMP_L, false);
		wcd938x_rx_clk_disable(codec);
		snd_soc_update_bits(codec,
			      ...
			      struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		wcd938x_rx_connect_port(codec, HPH_R, true);
		if (wcd938x->comp2_enable)
			wcd938x_rx_connect_port(codec, COMP_R, true);
		break;
	case SND_SOC_DAPM_POST_PMD:
		wcd938x_rx_connect_port(codec, HPH_R, false);
		if (wcd938x->comp2_enable)
			wcd938x_rx_connect_port(codec, COMP_R, false);
		wcd938x_rx_clk_disable(codec);
		snd_soc_update_bits(codec,
				WCD938X_DIGITAL_CDC_DIG_CLK_CTL,
				0x02, 0x00);
			      ...
			      int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		wcd938x_rx_connect_port(codec, LO, true);
		break;
	case SND_SOC_DAPM_POST_PMD:
		wcd938x_rx_connect_port(codec, LO, false);
		/* 6 msec delay as per HW requirement */
		usleep_range(6000, 6010);
		wcd938x_rx_clk_disable(codec);
		snd_soc_update_bits(codec,
 ...
 */
int wcd938x_mbhc_micb_adjust_voltage(struct snd_soc_codec *codec,
			      ...
			      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		ret = wcd938x_tx_channel_config(codec, w->shift, 1);
		...
		ret |= wcd938x_tx_channel_config(codec, w->shift, 0);
		break;
	case SND_SOC_DAPM_POST_PMD:
		switch (w->shift) {
		case 0:
...
int wcd938x_micbias_control(struct snd_soc_codec *codec,
				int micb_num, int req, bool is_dapm)
{
	...
	case MICB_PULLUP_ENABLE:
		if (!wcd938x->dev_up) {
			dev_dbg(codec->dev, "%s: enable req %d wcd device down\n",
				__func__, req);
			ret = -ENODEV;
			goto done;
		}
				...
				void *data)
{
	...
	struct snd_soc_codec *codec = wcd938x->codec;
	...
	case BOLERO_WCD_EVT_PA_OFF_PRE_SSR:
		snd_soc_update_bits(codec, WCD938X_ANA_HPH,
					0xC0, 0x00);
		snd_soc_update_bits(codec, WCD938X_ANA_EAR,
					0x80, 0x00);
		snd_soc_update_bits(codec, WCD938X_AUX_AUXPA,
					0x80, 0x00);
		break;
	...
	case BOLERO_WCD_EVT_SSR_UP:
		...
		mbhc = &wcd938x->mbhc->wcd_mbhc;
		ret = wcd938x_mbhc_post_ssr_init(wcd938x->mbhc, codec);
		if (ret) {
	...
	case BOLERO_WCD_EVT_CLK_NOTIFY:
		...
		break;
	default:
		dev_dbg(codec->dev, "%s: invalid event %d\n", __func__, event);
		break;
	}
	return 0;
}
					  ...
					  int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		wcd938x_micbias_control(codec, micb_num,
						MICB_ENABLE, true);
		break;
	case SND_SOC_DAPM_POST_PMU:
		/* 1 msec delay as per HW requirement */
 ...
 */
int wcd938x_get_codec_variant(struct snd_soc_codec *codec)
{
				   ...
				   struct snd_soc_codec *codec)
{
	...
	priv->entry = snd_info_create_subdir(codec_root->module,
					     "wcd938x", codec_root);
	if (!priv->entry) {
		dev_dbg(codec->dev, "%s: failed to create wcd938x entry\n",
			__func__);
		return -ENOMEM;
	}
	version_entry = snd_info_create_card_entry(card->snd_card,
						   ...
						   priv->entry);
	if (!version_entry) {
		dev_dbg(codec->dev, "%s: failed to create wcd938x version entry\n",
			__func__);
		return -ENOMEM;
	}
	...
	variant_entry = snd_info_create_card_entry(card->snd_card,
						   ...
						   priv->entry);
	if (!variant_entry) {
		dev_dbg(codec->dev, "%s: failed to create wcd938x variant entry\n",
			__func__);
		return -ENOMEM;
	}
...
static int wcd938x_soc_codec_probe(struct snd_soc_codec *codec)
{
	struct wcd938x_priv *wcd938x = snd_soc_codec_get_drvdata(codec);
	...
	wcd938x->codec = codec;
	snd_soc_codec_init_regmap(codec, wcd938x->regmap);

	variant = (snd_soc_read(codec,
			WCD938X_DIGITAL_EFUSE_REG_0) & 0x1E) >> 1;
	wcd938x->variant = variant;

	wcd938x->fw_data = devm_kzalloc(codec->dev,
	...
	ret = wcd938x_mbhc_init(&wcd938x->mbhc, codec, wcd938x->fw_data);
	if (ret) {
	...
	if (wcd938x->variant == WCD9380) {
		ret = snd_soc_add_codec_controls(codec, wcd9380_snd_controls,
	...
	if (wcd938x->variant == WCD9385) {
		ret = snd_soc_add_codec_controls(codec, wcd9385_snd_controls,
...
static int wcd938x_soc_codec_remove(struct snd_soc_codec *codec)
{
	struct wcd938x_priv *wcd938x = snd_soc_codec_get_drvdata(codec);

	if (!wcd938x) {
		dev_err(codec->dev, "%s: wcd938x is already NULL\n",
			__func__);
		return -EINVAL;
	}
	if (wcd938x->register_notifier)
...
static struct snd_soc_codec_driver soc_codec_dev_wcd938x = {
...
static int wcd938x_bind(struct device *dev)
{
	...
	ret = snd_soc_register_codec(dev, &soc_codec_dev_wcd938x,
				     NULL, 0);
	if (ret) {
...
static int wcd938x_add_slave_codecs(struct device *dev,
